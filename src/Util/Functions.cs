...
namespace Edu.Stanford.Nlp.Util
{
	...
	public class Functions
	{
		...
		private class ComposedFunction<T1, T2, T3> : Func<T1, T3>
		{
			internal Func<T2, T3> g;

			internal Func<T1, T2> f;

			public ComposedFunction(Func<T2, T3> g, Func<T1, T2> f)
			{
				this.g = g;
				this.f = f;
			}
		...
		public static Func<T1, T3> Compose<T1, T2, T3, _T3>(Func<T1, T2> f, Func<_T3> g)
		{
