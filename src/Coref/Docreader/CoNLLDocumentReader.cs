...
namespace Edu.Stanford.Nlp.Coref.Docreader
{
	...
	/// </remarks>
	/// <author>Angel Chang, Heeyoung Lee</author>
	public class CoNLLDocumentReader : IDocReader
	{
		/// <summary>A logger for this class</summary>
		private static readonly Redwood.RedwoodChannels logger = Redwood.Channels(typeof(Edu.Stanford.Nlp.Coref.Docreader.CoNLLDocumentReader));
		...
		private CoNLLDocumentReader.DocumentIterator docIterator;

		protected internal readonly IList<File> fileList;

		private int curFileIndex;

		private readonly CoNLLDocumentReader.Options options;

		private static readonly IHeadFinder chineseHeadFinder = new ChineseSemanticHeadFinder();

		public CoNLLDocumentReader(string filepath)
			: this(filepath, new CoNLLDocumentReader.Options())
		{
		}

		public CoNLLDocumentReader(string filepath, CoNLLDocumentReader.Options options)
		{
			...
			{
				this.fileList = Java.Util.Collections.EmptyList;
			}
			...
			{
				logger.Info("Reading " + fileList.Count + " CoNLL files from " + filepath);
			}
		}
		...
		public virtual CoNLLDocumentReader.CoNLLDocument GetNextDocument()
		{
			try
			{
				...
				if (docIterator == null)
				{
					docIterator = new CoNLLDocumentReader.DocumentIterator(curFile.GetAbsolutePath(), options);
				}
				...
				CoNLLDocumentReader.CoNLLDocument next = docIterator.Current;
		...
		{
			public bool printConLLLoadingMessage = true;
		...
		{
			private const long serialVersionUID = 6287339385357914531L;
			...
			public IList<IList<string[]>> sentenceWordLists = new List<IList<string[]>>();
		...
		private class DocumentIterator : AbstractIterator<CoNLLDocumentReader.CoNLLDocument>, ICloseable
		{
			...
			internal CoNLLDocumentReader.CoNLLDocument nextDoc;
			...
			public DocumentIterator(string filename, CoNLLDocumentReader.Options options)
			{
				// State
				this.options = options;
			...
			public override CoNLLDocumentReader.CoNLLDocument Current
			{
				get
				{
					if (nextDoc == null)
					...
					CoNLLDocumentReader.CoNLLDocument curDoc = nextDoc;
					nextDoc = ReadNextDocument();
					return curDoc;
				}
			}
			...
			public virtual void AnnotateDocument(CoNLLDocumentReader.CoNLLDocument document)
			{
				...
				for (int i = 0; i < sentences.Count; i++)
				{
					...
					foreach (Triple<int, int, string> nerSpan in nerSpans)
					{
						...
						if (t.GetSpan().GetSource() == startToken && t.GetSpan().GetTarget() == endToken)
						{
							...
							if (options.annotateTreeNer)
							{
								...
								{
									((CoreLabel)tlabel).Set(typeof(CoNLLDocumentReader.NamedEntityAnnotation), nerChunk);
								}
							}
						}
					}
					...
					foreach (Triple<int, int, string> corefSpan in corefSpans)
					{
						...
						if (options.annotateTreeCoref)
						{
							...
							if (tlabel is CoreLabel)
							{
								((CoreLabel)tlabel).Set(typeof(CoNLLDocumentReader.CorefMentionAnnotation), mention);
							}
						}
					}
				}
			...
			public virtual CoNLLDocumentReader.CoNLLDocument ReadNextDocument()
			{
				try
				{
					IList<string[]> curSentWords = new List<string[]>();
					CoNLLDocumentReader.CoNLLDocument document = null;
					for (string line; (line = br.ReadLine()) != null; )
					{
						...
						if (line.Length != 0)
						{
							if (line.StartsWith(docStart))
							{
								...
								document = new CoNLLDocumentReader.CoNLLDocument();
		...
		public class CorpusStats
		{
			...
			public virtual void Process(CoNLLDocumentReader.CoNLLDocument doc)
			{
				...
				foreach (string id in doc.corefChainMap.Keys)
				{
					...
					foreach (ICoreMap m in mentions)
					{
						...
						else
						{
							...
							if (tlabel is CoreLabel)
							{
								ICoreMap mention = ((CoreLabel)tlabel).Get(typeof(CoNLLDocumentReader.CorefMentionAnnotation));
						...
						{
							if (((CoreLabel)tlabel_1).ContainsKey(typeof(CoNLLDocumentReader.NamedEntityAnnotation)))
							{
								// walk up tree
								nerMentions++;
								...
								while (parent != null)
								{
									...
									{
										if (((CoreLabel)plabel).ContainsKey(typeof(CoNLLDocumentReader.NamedEntityAnnotation)))
										{
											logger.Info("NER Mention: " + m);
											ICoreMap parentNerChunk = ((CoreLabel)plabel).Get(typeof(CoNLLDocumentReader.NamedEntityAnnotation));
		...
		public static void Main(string[] args)
		{
			...
			if (ext != null)
			{
				options = new CoNLLDocumentReader.Options(".*" + ext + "$");
			}
			else
			{
				options = new CoNLLDocumentReader.Options();
			}
			options.annotateTreeCoref = true;
			options.annotateTreeNer = true;
			CoNLLDocumentReader.CorpusStats corpusStats = new CoNLLDocumentReader.CorpusStats();
			CoNLLDocumentReader reader = new CoNLLDocumentReader(filepath, options);
			int docCnt = 0;
			int sentCnt = 0;
			int tokenCnt = 0;
			for (CoNLLDocumentReader.CoNLLDocument doc; (doc = reader.GetNextDocument()) != null; )
			{
				...
				if (debug)
				{
					logger.Info("Document " + docCnt + ": " + anno.Get(typeof(CoreAnnotations.DocIDAnnotation)));
				}
				foreach (ICoreMap sentence in anno.Get(typeof(CoreAnnotations.SentencesAnnotation)))
				{
					if (debug)
					{
						logger.Info("Parse: " + sentence.Get(typeof(TreeCoreAnnotations.TreeAnnotation)));
					}
					if (debug)
					{
						logger.Info("Sentence Tokens: " + StringUtils.Join(sentence.Get(typeof(CoreAnnotations.TokensAnnotation)), ","));
					}
				...
				if (debug)
				{
					...
					foreach (string id in doc.corefChainMap.Keys)
					{
						logger.Info("Coref: " + id + " = " + StringUtils.Join(doc.corefChainMap[id], ";"));
					}
				}
			}
			fout.Close();
		...
		public virtual InputDoc NextDoc()
		{
			CoNLLDocumentReader.CoNLLDocument conllDoc = GetNextDocument();
			if (conllDoc == null)
			{
				return null;
			}
			...
			IDictionary<string, string> docInfo = MakeDocInfo(conllDoc);
		...
		private static IDictionary<string, string> MakeDocInfo(CoNLLDocumentReader.CoNLLDocument conllDoc)
		{
			IDictionary<string, string> docInfo = Generics.NewHashMap();
			docInfo["DOC_ID"] = conllDoc.documentID;
			docInfo["DOC_PART"] = conllDoc.partNo;
			docInfo["DOC_ID_PART"] = conllDoc.documentIdPart;
			docInfo["DOC_FILE"] = conllDoc.filename;
			return docInfo;
		}
		...
		public virtual IList<IList<Mention>> ExtractGoldMentions(CoNLLDocumentReader.CoNLLDocument conllDoc)
		{
			...
			foreach (string corefIdStr in corefChainMap.Keys)
			{
				...
				foreach (ICoreMap m in corefChainMap[corefIdStr])
				{
