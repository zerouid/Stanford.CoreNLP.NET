...
namespace Edu.Stanford.Nlp.Trees
{
	...
	public class SemanticHeadFinder : ModCollinsHeadFinder
	{
		...
		private static readonly Redwood.RedwoodChannels log = Redwood.Channels(typeof(Edu.Stanford.Nlp.Trees.SemanticHeadFinder));
		...
		public SemanticHeadFinder(bool noCopulaHead)
		...
		/// </param>
		public SemanticHeadFinder(ITreebankLanguagePack tlp, bool noCopulaHead)
			: base(tlp)
		{
		...
		private void RuleChanges()
		{
			...
			nonTerminalInfo["NP"] = new string[][] { new string[] { "rightdis", "NN", "NNP", "NNPS", "NNS", "NX", "NML", "JJR", "WP" }, new string[] { "left", "NP", "PRP" }, new string[] { "rightdis", "$", "ADJP", "FW" }, new string[] { "right", "CD" }, 
				new string[] { "rightdis", "JJ", "JJS", "QP", "DT", "WDT", "NML", "PRN", "RB", "RBR", "ADVP" }, new string[] { "rightdis", "VP", "VB", "VBZ", "VBD", "VBP" }, new string[] { "left", "POS" } };
		...
		private static readonly TregexPattern[] headOfCopulaTregex = new TregexPattern[] { TregexPattern.Compile("SBARQ < (WHNP $++ (/^VB/ < " + EnglishPatterns.copularWordRegex + " $++ ADJP=head))"), TregexPattern.Compile("SBARQ < (WHNP=head $++ (/^VB/ < "
		...
		private static readonly TregexPattern[] headOfConjpTregex = new TregexPattern[] { TregexPattern.Compile("CONJP < (CC <: /^(?i:but|and)$/ $+ (RB=head <: /^(?i:not)$/))"), TregexPattern.Compile("CONJP < (CC <: /^(?i:but)$/ [ ($+ (RB=head <: /^(?i:also|rather)$/)) | ($+ (ADVP=head <: (RB <: /^(?i:also|rather)$/))) ])"
			), TregexPattern.Compile("CONJP < (CC <: /^(?i:and)$/ [ ($+ (RB=head <: /^(?i:yet)$/)) | ($+ (ADVP=head <: (RB <: /^(?i:yet)$/))) ])") };

		private static readonly TregexPattern noVerbOverTempTregex = TregexPattern.Compile("/^VP/ < NP-TMP !< /^V/ !< NNP|NN|NNPS|NNS|NP|JJ|ADJP|S");
		...
		/// </remarks>
		private static readonly IPredicate<Tree> RemoveTmpAndAdv = null;
